...
try:
    ...
    create_block_mask, flex_attention = None, None
...
def _flex_attention_fn(inference=False):
    ...
    if key not in _FLEX_CACHE:
        ...
        # Check if hardware supports FLASH backend (requires Hopper SM90+ or Blackwell SM100+)
        is_sm90_plus = torch.cuda.is_available() and torch.cuda.get_device_capability(0)[0] >= 9
        use_flash_backend = inference and is_sm90_plus and os.environ.get("VDN_FORCE_TRITON", "0") != "1"
        if use_flash_backend:
            _FLEX_CACHE[key] = torch.compile(
                functools.partial(flex_attention, kernel_options={"BACKEND": "FLASH"}),
                dynamic=False)
        else:
            # Uses PyTorch's native Triton backend (supported across NVIDIA GPUs)
            _FLEX_CACHE[key] = torch.compile(flex_attention, dynamic=(not inference))
